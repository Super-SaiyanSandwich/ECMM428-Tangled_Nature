...
bool alpha[N];				    //helper list for implementing interspecies coupling matrix efficiently
double Jran1[N];			    //helper list for implementing interspecies coupling matrix efficiently
...
inline void initialise_model(int model_seed){
    ...
    for(int i=0; i<N; i++){
		( mt_rand() < theta ) ? alpha[i] = true : alpha[i] = false;
		Jran1[i] = oC*distribution(generator);
...
inline list<Species>::iterator kill(){
	double rand = mt_rand(); //replace with curand
	double sum = 0;
	for (list<Species>::iterator cur=ecology.begin(); cur != ecology.end(); ++cur){
...
int main(int argc, char *argv[]){
    ...
    do{
		generation<<<blocks,threads>>>(lgen);
		...
		//generation is over
		++t_gens;
		lgen = Npop / pKill; //recalculate generation length
		print_file(pop_file);		
