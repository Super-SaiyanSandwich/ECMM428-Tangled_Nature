...
#define blocks 1    // Number of GPU blocks to be used
#define threads 128 // Number of GPU threads per block to use

#define models 128 // blocks * threads

#define alpha_Arrays 5
#define J_Arrays 8

#define combinations 1120
...
//helper lists for implementing interspecies coupling matrix efficiently

bool *alpha[combinations];
double *JranA[combinations];
double *JranB[combinations];

int *Npop;        //total population tracker of each model
...
cudaMallocManaged(&Npop, models*sizeof(int));
std::fill(Npop, Npop + models, Npop_INITIAL);

curandState *d_state;
cudaMallocManaged(&d_state, sizeof(curandState));

cudaMallocManaged(&JranA, combinations * N * sizeof(double));
cudaMallocManaged(&JranB, combinations * N * sizeof(double));
cudaMallocManaged(&alpha, combinations * N * sizeof(double));
///////////////////////////
...
inline int random_CUDA_Species(curandState *curand_State){
	int idx = threadIdx.x + blockDim.x*blockIdx.x;

	return (int)(N * curand_uniform(curand_State + idx));
}
///////////////////////////
...
__global__
void setup_Kernel(curandState *state, int model_seed)
{
  	int idx = threadIdx.x+blockDim.x*blockIdx.x;
  	curand_init(model_seed, idx, 0, &state[idx]);
}

__global__
void setup_Helper_Arrays(curandState *state)
{
	double oC = sqrt(C);
	...
	for (int i = index; i < combinations; i += stride)
  	{
		for (int j =0; i<N; i++){
			(curand_uniform(state + idx) < THETA) ? alpha[i][j] = true : alpha[i][j] = false;
			JranA[i][j] = oC*curand_normal_double(state + idx);
			JranB[i][j] = oC*curand_normal_double(state + idx);
		}
  	}
}
...
__global__
void initialise_Model(curandState *state)
{
	random_CUDA_Species(state);
}
...
//
inline void initialise_System(int model_seed){

    // default_random_engine generator(model_seed + 123);
	// mt_generator = mt19937(model_seed);
	
	setup_Kernel<<<blocks, threads>>>(d_state, model_seed);

    setup_Helper_Arrays<<<blocks, threads>>>(d_state);
...
inline double calc_HI(list<Species>::iterator elem){
	...
	for (list<Species>::iterator cur=ecology.begin(); cur != ecology.end(); ++cur){ //loop over each species in ecology
		if(cur->bin_sID != elem->bin_sID){	                                        //no self interaction
			...
			if( alpha[s_inter] ){
				sum += Jran1[s_inter] * Jran2[cur->sID] * cur->population;	//inter species
			}
		}
	}
	return sum;
...
int main(int argc, char *argv[]){
    ...
    initialise_System(seed);
    int t = 0;                     //intra-generational counter
    double lgen = Npop[0] / pKill;    //length of initial generation

    string filename =  "CUDAplot_seed" + to_string(it);
