...
inline double calc_HI(list<Species>::iterator elem, bool *c, double *a, double *b){
	double sum = 0; 
...
inline double calc_H(list<Species>::iterator elem, bool *c, double *a, double *b){
	return calc_HI(elem)/Npop - mu*Npop;
}

//reproduction probability	
inline double poff(list<Species>::iterator elem, bool *c, double *a, double *b){
    return 1.0/(1.0 + exp(-calc_H(elem, c, a, b)));
}
...
int main(int argc, char *argv[]){
	...
	cudaMallocManaged(&cAlpha, N*sizeof(bool));

	cudaMallocManaged(&cRan1, N*sizeof(double));
  	cudaMallocManaged(&cRan2, N*sizeof(double));
    ...
    do{
		...
		pOff = poff(sID, N, cAlpha, cRan1, cRan2);
