...
int main(int argc, char *argv[]){
	...
	list<Species>::iterator killed_sID;
	list<Species>::iterator dormant_sID;
	list<Species>::iterator breeding_sID;
	list<Species>::iterator asleep_sID;
	list<Species>::iterator asleep_killed_sID;

    do{
		if(Apop > 0){
			killed_sID = kill_awake();       //choose an individual and kill with prob pkill

			if(Npop == 0){ break; }                     //if population totally killed off, end
			

			dormant_sID = killed_sID;
